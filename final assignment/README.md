...
- [IJssel River Room for the River Project](#ijssel-river-room-for-the-river-project)
  ...
  - [Modeling Pipeline](#modeling-pipeline)
    ...
    - [Step 2a: Uncertainty Analysis](#step-2a-uncertainty-analysis)
    - [Step 2b: Scenario Discovery \& Selection](#step-2b-scenario-discovery--selection)
      - [`:: Scenario_Discovery.ipynb`:](#-scenario_discoveryipynb)
      - [`:: open_exploration__scenario_diversity_scoring.py`:](#-open_exploration__scenario_diversity_scoringpy)
    - [Step 3: Multi-Scenario, Multi-Objective Robust Policy Search](#step-3-multi-scenario-multi-objective-robust-policy-search)
      - [`:: optimization__seeded_fixed_scenario.py`:](#-optimization__seeded_fixed_scenariopy)
      - [`:: Directed Search.ipynb`:](#-directed-searchipynb)
      - [`:: run_experiments.py`](#-run_experimentspy)
      - [`:: Policy Robustness.ipynb`:](#-policy-robustnessipynb)
...
* [output/](output) contains all output files produced in our analysis process, which are mostly CSVs or compressed tarballs of more CSVs. There is a table in Modeling Pipeline `(TODO: link)` containing a description of these files and naming their originating file. **NOTE**: Running our analysis files will overwrite these files. There is one case where these the contents of these files is not seeded, so will produce different results downstream. This is marked in comments and in Modeling Pipeline.
...
* The workbench interface file [problem_formulation](problem_formulation.py) was modified in two major ways. First, all model variables were passed a `name` parameter that removed periods and spaces, to prevent some small downstream issues in the EMA workbench. Second, the formulation options were changed. Explanation and justification of the provided cases can be found in that file and in our report.
...
* **These files are what to run to replicate our results. Their usage is described in Modelling Pipeline, below.**
...
### Step 2b: Scenario Discovery & Selection

**Files:** [Scenario_Discovery.ipynb](Scenario_Discovery.ipynb) & [open_exploration__scenario_diversity_scoring.py](open_exploration__scenario_diversity_scoring.py)
...
### Step 3: Multi-Scenario, Multi-Objective Robust Policy Search

**Files:** [optimization__seeded_fixed_scenario.py](optimization__seeded_fixed_scenario.py), [Directed Search.ipynb](Directed%20Search.ipynb), and [Policy Robustness.ipynb](Policy%20Robustness.ipynb)
...
**Purpose & Output:** Generates an EMA workbench-compatible results tarball called `robustness_results__1000_scenarios.tar.gz`. See Step 1 for a general description of this file.
...
**Required Input:** In this mode, the file requires a file called `output/policies__constraints_filtered__diverse_set_50.csv`, which is generated in the previous substep.
...
**Purpose & Output:** 
* Calculates a set of robustness metrics based on the experiment results from the previous substep.
...
**Instructions:** 
Open as a Jupyter Notebook and run from top to bottom or read the in-line analysis.

**Required Input:** Experiment results in `robustness_results__1000_scenarios.tar.gz` (or a similar file) and a CSV from one of the previous substeps capturing a policy set, from which to read and re-write the levers defining the final policy set.